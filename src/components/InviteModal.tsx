...
export function InviteModal({ onClose, t, userPhone, currentLang = 'EN' }: InviteModalProps) {
  ...
  const productionOrigin = typeof window !== 'undefined' ? window.location.origin : 'https://earnova-app-jrl6.vercel.app';
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'Join EarnLink',
        text: 'Join EarnLink and start earning today!',
        url: inviteLink,
      }).catch(console.error);
    } else {
      handleCopy();
        ...
        {showQrModal && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-black italic text-gray-900 leading-none uppercase tracking-tight">SCAN & JOIN EarnLink</h3>
              ...
              <div className="space-y-2">
                {qrCodeDataUrl && (
                  <a 
                    href={qrCodeDataUrl}
                    download={`earnlink-invite-qr-${inviteCode}.png`}
